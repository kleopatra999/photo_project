...
task :watch do
  begin
    ...
    script.watch('(css)/(src|lib)/.*') { system 'rake' }
...
task :css do
  ...
  if FileList["#{CSS_SRC}/*"].any?
    FileList["#{CSS_SRC}/*"].each do |f|
      fname = f.strip.split("/")[-1]
      css_name = fname.sub(/\.less/, ".css")
      if installed? "uglifycss"
        noExt = css_name.sub(/\.css/, "")
        puts "---> Uglifying #{css_name}"
        `uglifycss #{CSS_BIN}/#{css_name} > #{CSS_BIN}/#{noExt}.min.css`
      else
        fail "You need less! Install it by running: `npm install -g uglifycss`"
      end
    end
  end
end
